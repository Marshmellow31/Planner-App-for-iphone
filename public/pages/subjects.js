...
export async function renderSubjects(container, uid, profile) {
  container.innerHTML = `
    <div class="page-header">
      ...
      <button class="btn btn-primary btn-sm" id="btn-add-subject">+ Add</button>
    </div>
...
async function loadSubjects(container, uid, profile) {
  ...
  subjects.forEach((sub) => {
    ...
    card.className = "subject-card mb-sm clickable";
    ...
    card.querySelector(".btn-delete").addEventListener("click", async (e) => {
      e.stopPropagation();
      if (!confirm(`Delete subject "${sub.name}" and all its topics? Tasks will remain.`)) return;
...
// ── Subject Modal ─────────────────────────────────────────────
function openSubjectModal(uid, existing, onSave) {
  ...
  backdrop.innerHTML = `
    <div class="drawer" style="max-width:480px;margin:0 auto">
      ...
      <div class="modal-actions">
        ...
        <button class="btn btn-primary" id="sub-save">${isEdit ? "Save" : "Create"}</button>
      </div>
    </div>
  `;
